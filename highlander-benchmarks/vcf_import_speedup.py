...
process_max = 3 # Number maximal of imports to highlander we allow at the same time. Should be at least 1 or you will have some problems
...
def jarAsynchronous(*args):
    env = dict(os.environ)
    env['JAVA_OPTS'] = '-d64 -Xms'+str(ram_max-1)+'g -Xmx'+str(ram_max)+'g'
    p = subprocess.Popen(['java', '-jar']+list(args), env=env)
    return p
...
for analyse in analyses:
    ...
    max_vcf_step = 5# TODO enlever cette ligne après la fin des tests
    for first_sample in xrange(1, analyse[1], max_vcf_step):    
        ...
        # 3. Import the vcf files generated to highlander thanks to dbBuilder.jar (made by Raphaël Helaers)
        print("3. Importing the different samples ("+str(first_sample)+" -> "+str(first_sample+max_vcf_step)+").")
        error = 0            
        process_list = []
        for i in xrange(first_sample, first_sample+max_vcf_step):            
            ...
            try: 
                process_list.append(jarAsynchronous(*args))
                result = True
            except Exception as e:
                result = False
                print(e)
        
            # If we have enough processes, we wait for them
            if len(process_list) >= process_max:
                for p in process_list:
                    p.wait()
                process_list = []
                    
        # We finish the last processes
        ...
        for i in xrange(first_sample, first_sample+max_vcf_step, process_max):  
            path_to_file = vcf_destination_path+"r_"+analyse[0]+"_s"+str(max_vcf_step)+"_"+str(starting_sample + first_sample)+"."+str(i-first_sample)
